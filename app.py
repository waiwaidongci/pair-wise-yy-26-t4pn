...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path=="/api/extension-requests": return self._json(201,{"ok":True,"id":self.db.request_extension(int(b.get("report_id",0)),str(b.get("new_deadline","")),str(b.get("reason","")),int(b.get("coordinator_id",0)))})
            if len(parts)==4 and parts[:2]==["api","extension-requests"] and parts[3]=="vote": self.db.vote_extension(int(parts[2]),int(b.get("user_id",0)),str(b.get("decision","")),str(b.get("comment",""))); return self._json(200,{"ok":True})
            if len(parts)==4 and parts[:2]==["api","extension-requests"] and parts[3]=="review": self.db.review_extension(int(parts[2]),int(b.get("coordinator_id",0)),str(b.get("decision","")),str(b.get("note",""))); return self._json(200,{"ok":True})
            if len(parts)==4 and parts[:2]==["api","reports"] and parts[3]=="summary": self.db.update_summary(int(parts[2]),str(b.get("summary","")),int(b.get("user_id",0))); return self._json(200,{"ok":True})
            if len(parts)==4 and parts[:2]==["api","reports"] and parts[3]=="versions": self.db.set_affected_versions(int(parts[2]),list(b.get("versions",[])),int(b.get("user_id",0)),str(b.get("version_details",""))); return self._json(200,{"ok":True})
